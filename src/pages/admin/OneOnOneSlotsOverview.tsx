...
import { fetchOneOnOneSummaryStats, combineSlotsData } from '@/lib/oneOnOneService';
import type { SlotOverview } from '@/lib/slotsService';

type OneOnOneSlotsOverview = SlotOverview;

export function OneOnOneSlotsOverview() {
  const { toast } = useToast();
  const [data, setData] = useState<OneOnOneSlotsOverview[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const firestoreSlots = await listSlotsAvailability('oneOnOne');
      const summaryStats = await fetchOneOnOneSummaryStats();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">1on1 Slots Overview</h2>
          <p className="text-muted-foreground">Statistics on 1on1 slot allocation and attendance</p>
        </div>
...
export default OneOnOneSlotsOverview;
