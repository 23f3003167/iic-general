...
import { fetchBehavioralSummaryStats, combineSlotsData } from '@/lib/behavioralService';
import type { SlotOverview } from '@/lib/slotsService';

type BehavioralSlotsOverview = SlotOverview;

export function BehavioralSlotsOverview() {
  const { toast } = useToast();
  const [data, setData] = useState<BehavioralSlotsOverview[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const firestoreSlots = await listSlotsAvailability('behavioral');
      const summaryStats = await fetchBehavioralSummaryStats();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Behavioral Slots Overview</h2>
          <p className="text-muted-foreground">Statistics on behavioral slot allocation and attendance</p>
        </div>
...
export default BehavioralSlotsOverview;
